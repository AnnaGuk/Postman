import {getMethod, postMethod, putMethod, patchMethod, deleteMethod} from "./methods.mjs";
...
const addRow = row => {row.parentElement.appendChild(createBodyRowHtml())};
...
const sendRequest = (methodSelectTag, inputTag, responseBoxTag, bodyRows) => {
    ...
    switch (methodSelectTag.value) {
        case "get":
            getMethod(inputTag.value)
                .then(response => showResults(response, responseBoxTag));
            break;
        case "post":
            const dataPost = serializeData(bodyRows);
            postMethod(inputTag.value, dataPost)
                .then(response => showResults(response, responseBoxTag));
            break;
        case "put":
            const dataPut = serializeData(bodyRows);
            putMethod(inputTag.value, dataPut)
                .then(response => showResults(response, responseBoxTag));
            break;
        case "patch":
            const dataPatch = serializeData(bodyRows);
            patchMethod(inputTag.value, dataPatch)
                .then(response => showResults(response, responseBoxTag));
            break;
        case "delete":
            deleteMethod(inputTag.value)
                .then(response => showResults(response, responseBoxTag));
            break;
...
export const createPostmanAppGui = (config) => {
    ...
    const formTag = createHtmlTag({tagName: "section", classNames: ["box-form"]});
    const methodSelectTag = createHtmlTag({
        tagName: "select",
        event: {type: 'change', cb: () => {handleMethodsChange(bodyRows, methodSelectTag)}}
    });
    config.methods.forEach(method => {
        const methodTag = createHtmlTag({tagName: "option", text: method, value: method});
    ...
    const responseBoxTag = createHtmlTag({tagName: "pre", classNames: ["box-response"]});
